...
public static class GetUserByMailNicknameEndpoint
{
    public static WebApplication RegisterEndpoint(this WebApplication app)
    {
        app.MapGet("/getusers/by-mailnickname/{id}", async (string id, [FromServices] IGraphService graphService, HttpRequest request, HttpResponse response, HttpContext httpContext) => 
        {
            ...
            var user = await graphService.GetUserByMailNickname(id, getManagerBool);
