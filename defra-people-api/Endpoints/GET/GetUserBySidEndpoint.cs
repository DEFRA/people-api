...
public static class GetUserBySidEndpoint
{
    public static WebApplication RegisterEndpoint(this WebApplication app)
    {
        app.MapGet("/getusers/by-sid/{id}", async (string id, [FromServices] IGraphService graphService, HttpRequest request, HttpResponse response, HttpContext httpContext) => 
        {
            ...
            var user = await graphService.GetUserBySID(id, getManagerBool);
